...
| **CLI Native AOT publishing** | Both `AI.Sentinel.ClaudeCode.Cli` and `AI.Sentinel.Copilot.Cli` should publish with `<PublishAot>true</PublishAot>` to drop cold start from ~300 ms to ~30 ms — the hook binaries run per tool call, so every ms matters in-IDE. JSON source-gen contexts (`HookJsonContext`, `CopilotHookJsonContext`) are already AOT-ready; the remaining work is (a) flip the MSBuild flag, (b) verify no `IL2026`/`IL3050` warnings from `AddAISentinel` + ZeroAlloc source generators, (c) test the published AOT binaries end-to-end. |
| **Hook adapter `SENTINEL_HOOK_VERBOSE` env var** | Original design called for `SENTINEL_HOOK_VERBOSE=1` to emit the detection-reason JSON on stderr for debugging. Not implemented in the v0.1.0 ClaudeCode/Copilot CLIs. Low-effort follow-up: read the env var in `HookConfig.FromEnvironment`, wire into both `Program.cs` files. |
| **Prompt-only scan mode on `SentinelPipeline`** | Hook adapters currently run the full two-pass scan even though there's no real LLM response — `HookPipelineRunner.NullChatClient` returns a placeholder string tuned to avoid `OPS-01` false positives (regression-tested in `HookAdapterTests.NullResponseText_TriggersNoDetectors`). Cleaner: add `ScanPromptOnlyAsync(messages, ct)` to `SentinelPipeline` that skips the response scan entirely. Removes the placeholder-string workaround and avoids future detectors tripping on it. |
