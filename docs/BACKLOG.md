...
| **Add eviction to `SentinelPipeline._rateLimiters`** | The `ConcurrentDictionary<string, RateLimiter>` grows unbounded — one entry per unique session key for the pipeline lifetime. Same category as `DeduplicatingAlertSink._seen`. Add a background sweeper or `MemoryCache` with sliding expiry |
