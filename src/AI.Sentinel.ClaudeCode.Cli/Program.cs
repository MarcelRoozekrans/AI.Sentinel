...
public static class Program
{
    ...
    /// <summary>
    /// Builds the DI provider with optional approval bindings. Throws-not-supports (returns
    /// exit=1 with a stderr message) for Sqlite/EntraPim until Task 5.6 adds the project refs;
    /// None and InMemory share a code path because <c>AddAISentinel</c> auto-registers
    /// <c>InMemoryApprovalStore</c> when any binding has an <see cref="ApprovalSpec"/>.
    /// </summary>
        ...
        ApprovalConfig? approvalConfig)
    {
        ...
        // Lambda capture: AddAISentinel invokes the callback synchronously, so backendKind is
        // set before we read it below.
