...
public class AuthorizationTests
{
    ...
    public async Task PreToolUse_DenyByPolicy_ReturnsBlock()
    {
        ...
        // AdminOnlyPolicy falls back to the default 'policy_denied' code (DefaultToolCallGuard
        // canonicalises AuthorizationFailure.DefaultDenyCode → "policy_denied").
    ...
    {
        // Phase 3 Task 3.1: policies that return a structured Code (e.g. 'tenant_inactive') must
        // surface that code verbatim in the operator-facing receipt so log readers can correlate
        // CLI output with audit rows (where AuditEntry.PolicyCode carries the same value).
        var services = new ServiceCollection();
        services.AddAISentinel(o =>
        {
            o.OnCritical = SentinelAction.Quarantine;
